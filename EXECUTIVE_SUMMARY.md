...
A pre-registered empirical audit of sales-closing techniques. Phase 1 (Stage-1 screening) complete. Phase 2 (extraction) at **44 records** across Frontiers + IRSP + MDPI + APA-OA + survivor-technique-targeted + HIGH-confidence PMC subsets. **Six per-technique posteriors**: commitment-consistency (k=2 μ=0.590), regulatory-fit (k=3 μ=0.484), extreme-anchor (k=2 μ=0.435), gain-framing (k=9 μ=0.354), loss-framing (k=7 μ=0.327), social-proof (k=2 μ=0.682). Five of six have credible intervals cleanly excluding zero AND exceeding the d=0.2 practical-significance threshold with P>0.99. Sensitivity analyses (`scripts/sensitivity_analysis.py`) now pass stability tests across high-RoB exclusion, log-OR-conversion exclusion, AND leave-one-out drops after the atlas-009 parsing-artifact fix (gain-framing μ corrected from 0.474 → 0.354, τ collapsed from 0.301 → 0.045). OA-extractability classifier (`scripts/classify_oa_extractability.py`) shows only 8 HIGH-confidence OA records remain unextracted — Phase 2 has saturated the clean-OA subset. Production R/brms run pending Marion-action on R/brms/cmdstanr install.
