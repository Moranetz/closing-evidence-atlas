...
PRISMA 2020 flow diagram for the current Atlas state (post-commit `5b48d60`) is in `figures/prisma_flow.svg` — generated by `scripts/prisma_flow.py`. The diagram shows the pipeline from 12,416 raw records → 11,785 unique → 8,839 explicitly classified → 572 included → 141 OA → 44 extracted → 20 Phase-3-eligible across 6 techniques. The figure is regenerated after each Phase 2 expansion that materially shifts the funnel; the script reads pre-registered counts hard-coded against the protocol commit hash.
